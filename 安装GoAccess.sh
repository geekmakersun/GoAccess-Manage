...
readonly GEOIP_CITY_URL="https://git.io/GeoLite2-City.mmdb"
readonly GEOIP_ASN_URL="https://git.io/GeoLite2-ASN.mmdb"
readonly GEOIP_MIRROR_URLS=(
    "https://github.com/P3TERX/GeoLite.mmdb/raw/download/GeoLite2-City.mmdb"
    "https://cdn.jsdelivr.net/gh/P3TERX/GeoLite.mmdb@download/GeoLite2-City.mmdb"
    "https://fastly.jsdelivr.net/gh/P3TERX/GeoLite.mmdb@download/GeoLite2-City.mmdb"
)
...
fix_git_permissions() {
    ...
    chown -R $WWW_USER:$WWW_GROUP "$SCRIPT_DIR"
    log_success "✓ 完成"
...
download_geoip_database() {
        ...
        if download_with_retry "$url" "$output_file"; then
            if [ -f "$output_file" ] && [ -s "$output_file" ]; then
                ...
                if [ "$file_size" -gt 1000000 ]; then
                    ...
                    log_warning "下载的文件过小 ($file_size bytes)，可能不完整"
                    rm -f "$output_file"
                fi
            fi
        fi
    ...
    log_warning "所有镜像源下载失败，尝试备用方案..."
    
    if check_command geoipupdate; then
        log_info "检测到 geoipupdate 工具，尝试使用它更新数据库..."
        if geoipupdate 2>/dev/null; then
            if [ -f "$output_file" ]; then
                log_success "通过 geoipupdate 更新成功"
                return 0
            fi
        fi
    fi
...
if [ -f "$GEOIP_CITY_DB" ]; then
    log_info "GeoLite2-City 数据库已存在，跳过下载"
else
    log_info "开始下载 GeoLite2-City 数据库..."
    if download_geoip_database "$GEOIP_CITY_DB"; then
        log_success "GeoLite2-City 数据库下载成功"
    else
        log_warning "GeoLite2-City 数据库下载失败，但不影响 GoAccess 基本功能"
        log_info "提示: 可以手动下载数据库文件到 $GEOIP_CITY_DB"
    fi
fi

if [ -f "$GEOIP_ASN_DB" ]; then
    log_info "GeoLite2-ASN 数据库已存在，跳过下载"
else
    log_info "开始下载 GeoLite2-ASN 数据库..."
    if download_geoip_database "$GEOIP_ASN_DB"; then
        log_success "GeoLite2-ASN 数据库下载成功"
    else
        log_warning "GeoLite2-ASN 数据库下载失败，但不影响 GoAccess 基本功能"
    fi
fi
...
if [ -f "$GEOIP_CITY_DB" ]; then
    ...
    chown root:root "$GEOIP_CITY_DB"
fi
if [ -f "$GEOIP_ASN_DB" ]; then
    chmod 644 "$GEOIP_ASN_DB"
    chown root:root "$GEOIP_ASN_DB"
fi
